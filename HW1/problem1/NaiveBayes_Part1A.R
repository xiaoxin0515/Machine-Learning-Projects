# Time: 09/02/2018
# Author: Yuan Gao
# This script is used to build a Naive Bayes model for Pima Indians dataset
library(plyr)
...
param_estimate <- function(data,y){
  ...
  avg <- mean(data[data[,2]==y,1])
  variance <- var(data[data[,2]==y,1])
...
freq_cal <- function(input){
  ...
  # input: a n*1 vector
  # return a dataframe contains frequency of each unique elementS
  res <- count(input)
  return(res)
}
